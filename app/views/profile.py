...
from ..serializers import UserProfileImageSerializer, UserProfileSerializer, UserProfileUpdateSerializer
...
class ProfileImageAPIView(APIView):
    ...
    def get(self, request):
        ...
        serializer = self.serializer_class(profile_images, many=True, context={"request": request})
...
class UserProfileAPIView(APIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = UserProfileUpdateSerializer(user.first(), data=request.data, partial=True, context={"request": request})
        response_status, result, message = serialized_response(serializer, message)
        status_code = status.HTTP_200_OK if response_status else status.HTTP_400_BAD_REQUEST
        return custom_response(response_status, status_code, message, result)
